...
library(pacman)
p_load(sf, shiny, spData, leaflet, tidyverse, spDataLarge)

# Based on input coordinates finding the nearest bicycle points
ui <- fluidPage(
  
  # Application title
  titlePanel("Closest available bike in London"),
  ...
  fluidRow (
    ...
    column(4, numericInput("num", "How many bikes are you looking for?", value  =1, step  =1))
    ),
...
server <- function(input, output) {
  #centring the leaflet map onto london
